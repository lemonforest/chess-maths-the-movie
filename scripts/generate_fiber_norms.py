...
def pawn_adj() -> np.ndarray:
    """Symmetrized pawn adjacency: union of both colours' single-step
    moves. Vertical one-square advance (both directions merged) plus
    the four capture diagonals (both colours). The two-square first-
    move advance is excluded because it's position-dependent (only
    from starting ranks); everything here must be a static property
    of the square.

    This breaks the D4 symmetry the other piece fields enjoy — 90°
    rotation would turn vertical advances into horizontal ones, which
    pawns don't have — but Z2 x Z2 (vertical-axis reflection,
    horizontal-axis reflection, and their composite 180° rotation)
    still applies. The verification step exercises the smaller group.
    """
    A = _empty_adj()
    moves = [
        (-1,  0), ( 1,  0),       # vertical step (both colours collapsed)
        (-1, -1), (-1,  1),       # backward-diagonal captures
        ( 1, -1), ( 1,  1),       # forward-diagonal captures
    ]
    for r in range(N):
        for c in range(N):
            for dr, dc in moves:
...
# V3 is derived from the formally D4-symmetric non-trivial pieces only.
# Pawn is a lower-symmetry direction-collapsed approximation (see
# `pawn_adj`) — including it in the basis would mix Z2xZ2 content into
# the shared fiber directions and break bishop-queen parallelism. So
# pawn gets projected onto the existing N/B/Q/K-derived V3 instead.
...
# Subset of PIECE_ORDER that enjoys full D4 symmetry; pawn has only
# Z2 x Z2. Used by the verification step.
D4_PIECES = ["knight", "bishop", "rook", "queen", "king"]
...
def _is_klein4_symmetric(vec: np.ndarray, atol: float = 1e-9) -> bool:
    """Check the Z2 x Z2 subgroup of D4 generated by the two axis
    reflections (and their product, 180° rotation). Used for the
    direction-collapsed pawn field, which does NOT have full D4."""
    all_perms = _d4_perms()
    # Index mapping follows the order in _d4_perms: 0=id, 1=rot90,
    # 2=rot180, 3=rot270, 4=vert-axis, 5=horiz-axis, 6=transpose,
    # 7=anti-diag. The Klein-4 subgroup we care about is
    # {id, rot180, vert-flip, horiz-flip} = indices 0, 2, 4, 5.
    for idx in (0, 2, 4, 5):
        if not np.allclose(vec, vec[all_perms[idx]], atol=atol):
            return False
    return True
...
def verify(values: Dict[str, np.ndarray]) -> None:
    ...
    # Gate 2-6: D4 symmetry on each full-symmetry piece (and trivially on rook).
    for piece in D4_PIECES:
        assert _is_d4_symmetric(values[piece]), f"{piece} field not D4-symmetric"

    # Gate 7: pawn has Z2 x Z2 symmetry (axis reflections + 180° rotation)
    # but NOT full D4 — 90° rotation would map its vertical advances to
    # horizontal, which pawns don't have. If the field came out D4-
    # symmetric, something's wrong with the adjacency.
    assert _is_klein4_symmetric(values["pawn"]), (
        "pawn field not Z2xZ2-symmetric — adjacency is broken"
    )
    # And confirm it's NOT accidentally D4 — that would mean we've
    # unwittingly symmetrized in the horizontal direction too.
    # (Skip this check if pawn is identically zero, because zero is
    # trivially in every symmetry group.)
    if np.linalg.norm(values["pawn"]) > 1e-9:
        assert not _is_d4_symmetric(values["pawn"]), (
            "pawn field came out D4-symmetric, which means the adjacency "
            "wasn't actually direction-collapsed — fix pawn_adj()."
        )

    # Gate 8: bishop-queen proportionality (queen = bishop + rook; rook's
    ...
    # Gate 9: knight has sqrt(degree)-like corner < edge < center structure.
...
def main() -> int:
    ...
    doc = {
        ...
        "description": (
            ...
            "Static property of the chess rules. The V3 basis is derived "
            "from the D4-symmetric non-trivial pieces (knight, bishop, "
            "queen, king); pawn is a direction-collapsed approximation "
            "projected onto the same basis (Z2xZ2 symmetry, not D4)."
        ),
        ...
        "pawn_is_direction_collapsed": True,
